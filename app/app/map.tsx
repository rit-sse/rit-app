    ...
    <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
      ...
      width: buttonWidth, height: 3.0*buttonWidth+2.0*buttonSpacing}}>
        ...
        <View style={[{bottom: 0}, allButtonStyling]}>
          <BuildingIcon onPress={() => {showBuildingMenu(setModalVisible)}} style={iconStyle} fill={"#000"}/>
        </View>
        
      </View>

      <View style={{width:"100%",height:Dimensions.get('screen').height / 2}}>
        <ScrollView pagingEnabled scrollEnabled={modalVisible} showsVerticalScrollIndicator={false} style={{width:"100%",height:Dimensions.get('screen').height / 2,flex:1}}>
    {/* so here are your screens, for example first camera second images */}
          {/* <Page title={'PAGE 1 '} index={0} />
          <Page title={'PAGE 2'} index={1} /> */}
          <View style={{width:"100%",height:Dimensions.get('screen').height / 2,backgroundColor:"#ff000000"}}>
            <Text style={{}}>{modalVisible}</Text>
          </View>
          <View style={{width:"100%",height:Dimensions.get('screen').height / 2,backgroundColor:"#00ff00"}}>
...
function showBuildingMenu(setModalVisible:Function) {
