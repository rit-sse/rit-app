...
function parseTimeToday(timeStr: string): Date {
  const now = new Date();
  const match = new RegExp(/(\d{1,2})(?::(\d{2}))?\s*([ap])\.?\s*m?\.?/i).exec(
    timeStr,
  );
...
function resolveRunTimeMinutes(
  rawTime: string,
  serviceWindowCrossesMidnight: boolean,
): number | null {
...
function resolveCurrentTimeMinutes(
  route: RouteSchedule,
  currentTime: Date,
): number {
  const minutesNow = getMinutesNow(currentTime);
...
function findActiveRunIndex(
  ...
  for (let runIndex = 0; runIndex < firstStop.times.length; runIndex++) {
    const runMinutes = route.stops
      ...
      .map((time) =>
        resolveRunTimeMinutes(
          time,
          route.serviceWindow?.crossesMidnight ?? false,
        ),
      )
    ...
    const lastRunMinute =
      Math.max(...runMinutes) + RUN_COMPLETION_GRACE_MINUTES;
...
export function inferSchedule(route: RouteSchedule): InferredSchedule | null {
  ...
  const currentStopIndex = findCurrentStopIndex(
    route.stops,
    currentTime,
    activeTimeIndex,
  );
...
export function getNextStop(
  route: RouteSchedule,
): { stop: InferredStop; eta: number } | null {
