...
const MENU_CODES: Record<
  string,
  {
    accountId: number;
    locationId: number;
    mealPeriodIds: { [key: string]: number };
  }
> = {
  "ctrl-alt-deli": {
    accountId: 6,
...
function createMenuAPIURL(store: string, mealPeriodId: number): string {
  ...
  const baseUrl =
    "https://apiservicelocatorstenantrit.fdmealplanner.com/api/v1/data-locator-webapi/20/meals";
...
export async function GET(req: Request, res: Response) {
  ...
  if (
    req.query["store"] &&
    VALIDSTORES.includes(req.query["store"].toString())
  ) {
    console.log(
      "Processing menu request for store:",
      req.query["store"].toString(),
      "and meal period:",
      req.query["mealPeriod"]?.toString() || "default",
    );
    let inCache = await scrapeCache.inCache(
      `dining-menu-${req.query["store"].toString()}_${req.query["mealPeriod"]?.toString() || "default"}`,
    );
    let isExpired = await scrapeCache.isExpired(
      `dining-menu-${req.query["store"].toString()}_${req.query["mealPeriod"]?.toString() || "default"}`,
    );
    if (inCache && !isExpired && !MENU_DEBUG) {
      console.log(
        "Serving menu data from cache for store:",
        req.query["store"].toString(),
        "and meal period:",
        req.query["mealPeriod"]?.toString() || "default",
      );
      res.send(
        await scrapeCache.getCache(
          `dining-menu-${req.query["store"].toString()}_${req.query["mealPeriod"]?.toString() || "default"}`,
        ),
      );
      return;
    ...
    let data = await fetch(
      createMenuAPIURL(
        req.query["store"].toString(),
        MENU_CODES[req.query["store"].toString()].mealPeriodIds[
          req.query["mealPeriod"]?.toString() || "default"
        ],
      ),
    );
    ...
    for (let item of menuData) {
      ...
      formattedMenu.push({
        ...
        conditionals: item["recipeProductDietaryName"]
          .split(",")
          .map((cond: string) => cond.trim())
          .filter((cond: string) => cond !== ""),
      });
  ...
  res
    .status(400)
    .send({ error: "Invalid or missing 'store' query parameter." });
}
