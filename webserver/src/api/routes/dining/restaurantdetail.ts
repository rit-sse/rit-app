...
export async function GET(req: Request, res: Response) {
  ...
  if (
    (await scrapeCache.inCache(`restaurantdetail_${restaurantCode}`)) &&
    !(await scrapeCache.isExpired(`restaurantdetail_${restaurantCode}`))
  ) {
  ...
  const scrape = await fetch(
    `https://www.rit.edu/dining/location/${restaurantCode}`,
    {
      headers: {
        "User-Agent":
    ...
    $(el)
      ...
      .map((k, dayEl) => {
        ...
        if (!Object.keys(restaurantData.hoursOfOperations).includes(dayName)) {
          restaurantData.hoursOfOperations[dayName] =
            hours?.map((e) => e.trim()) || [];
        }
      });
  });
  ...
  await scrapeCache.setCache(
    `restaurantdetail_${restaurantCode}`,
    restaurantData,
  );
  await res.send(
    await scrapeCache.getCache(`restaurantdetail_${restaurantCode}`),
  );
}
