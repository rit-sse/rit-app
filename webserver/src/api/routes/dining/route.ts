...
export async function GET(req: Request, res: Response) {
  ...
  if (
    ((await scrapeCache.inCache("dining_locations")) &&
      !(await scrapeCache.isExpired("dining_locations"))) ||
    DINING_DEBUG
  ) {
    ...
    if (r.code) {
      ...
      $storeScrape('div[class="week-display"]').map((j, el) => {
        $(el)
          ...
          .map((k, dayEl) => {
            let dayName = $storeScrape(dayEl)
              .find('div[class="day-name"]')
              .text()
              .trim();
            let hours = $storeScrape(dayEl)
