...
export const GET = async (req: Request, res: Response) => {
  ...
  if (
    (await scrapeCache.getCache("rit_events")) &&
    !(await scrapeCache.isExpired("rit_events"))
  ) {
    return res
      .header("Content-Type", "application/json")
      .send(await scrapeCache.getCache("rit_events"));
  }

  const events = await (
    await fetch(
      "https://campusgroups.rit.edu/mobile_ws/v17/mobile_events_list?range=0&limit=40",
    )
  ).json();
