...
export async function GET(req: Request, res: Response) {
  if (
    (await scrapeCache.getCache("club_infos")) &&
    !(await scrapeCache.isExpired("club_infos"))
  ) {
    return res
      .header("Content-Type", "application/json")
      .send(await scrapeCache.getCache("club_infos"));
  }
  let clubPage = await (
    await fetch(
      "https://campusgroups.rit.edu/club_signup?view=all&group_type=9999",
    )
  ).text();
  ...
  for (let club of clubList) {
    ...
    let clubType = $(club)
      .find(".h5.media-heading.grey-element")
      .text()
      .trim()
      .replaceAll("\t", " ")
      .replaceAll("\n", " ");
    ...
    let fullClub = false;
    $(club)
      .find(".checkbox.checkbox-cg--group")
      .toArray()
      .forEach((p) => {
        if (
          $(p)
    ...
    // Get image
    let clubImage =
      $(club).find(".media-object.media-object--bordered").attr("src") || "";
    ...
    $(club)
      .find(".listing-element__title-block")
      .text()
      .includes("Group password")
      ? (passwordLocked = true)
