...
const buildingCoordinates: {[buildingCode: string]: {lat: number, lon: number}} = {
...
export async function GET(req: Request, res: Response) {
    ...
    if (await scrapeCache.inCache("buildings") && !(await scrapeCache.isExpired("buildings"))) {
    ...
    const scrape = await fetch("https://www.rit.edu/facilitiesmanagement/buildings-maps");
    ...
    // Data
    var storedData: {buildingNumber: string, buildingAbbreviations: string, buildingName: string, buildingImage: string, floorUrls: {[floorName: string]: string}}[] = []
    await Promise.all($('.view-content').find('tbody').find('tr').map(async (i, el) => {
        // Extract building data from each table row
        const buildingNumber = $(el).find('td[headers="view-field-building-number-table-column"]').text().trim();
        const buildingAbbreviations = $(el).find('td[headers="view-field-abbreviation-s-table-column"]').text().trim();
        ...
        // Get building image
        buildingImage = "https://www.rit.edu" + $b('.field--name-field-building-image').find('img').attr("src") || "";
        console.log(`Building Image for ${buildingName}: ${buildingImage}`);

        $b('tbody').find('tr').map((j, row) => {
        ...
        storedData.push({buildingNumber, buildingAbbreviations, buildingName, buildingImage, floorUrls})
    }))
